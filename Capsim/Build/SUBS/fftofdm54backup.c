...
/*  ************************************************************************
  ...
  general input N = 2^n, n is any integer. 
...
void bri(int * brIndexPtr, int fftSize);

double mylog2(double x);

void twiddleVectors(float * cosVectorPtr, float * sinVectorPtr, int twiddleSize, int expSign);
...
log2FftSize=(int)mylog2((double)fftSize);
...
for (i=0; i<twiddleSize; i++) {
   ...
   cosVectorPtr[i]=mycos_32((float)(expSign*PI*i/twiddleSize));

   sinVectorPtr[i]=mysin_32((float)(expSign*PI*i/twiddleSize));

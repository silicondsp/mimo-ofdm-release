...
case PARAM_INIT: 
    {
     ...
     char   *ptype2 = "file";
     char   *pval2 = "cxmimo_2x2_svd_fp0.taps";
...
case USER_INIT: 
	...
	if((numberRxAntennas = NO_INPUT_BUFFERS()) < 1) {
                fprintf(stderr,"cxmimo_2x2_svd_fp: no input buffers\n");
		return(2);
	}
	if((numberStreams = NO_OUTPUT_BUFFERS()) < 1) {
              fprintf(stderr,"cxmimo_2x2_svd_fp: no output buffers\n");
		return(3);
	}

	if(numberStreams < 2) {
              fprintf(stderr,"cxmimo_2x2_svd_fp: Minimum 2 output Buffers required\n");
		return(3);
	}
        ...
        if(numberStreams != numberTxAntennas) {
               fprintf(stderr,"cxmimo_2x2_svd_fp: output streams must match number Tx antennas fo now.\n");
		return(4);
       ...
       fp=fopen(HMatrixFileName,"r");
        if(!fp) {
            fprintf(stderr,"cxmimo_2x2_svd_fp: Could not open H Matrix file %s.\n",HMatrixFileName);
                return(5);
        }
        ...
        if(nTx != numberTxAntennas ) {
            fprintf(stderr,"cxmimo_2x2_svd_fp: Param Tx Antenna does not match in H Matrix file s.\n");
                return(6);

        }

        if(nRx != numberRxAntennas ) {
            fprintf(stderr,"cxmimo_2x2_svd_fp: Rx paths do  not match number of Rx antennas in  H Matrix file s.\n");
                return(7);
        ...
        if(!xre_P || !xim_P || !yre_P || !yim_P ) {
            fprintf(stderr,"cxmimo_2x2_svd_fp: Could not allocate space.\n");
                return(8);
...
if(h_P==NULL) {
           fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate H matrix\n");
	   return(8);
...
if(H_P==NULL) {
              fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate single  H matrix\n");
	     return(11);
...
if(U_P==NULL) {
              fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate  Matrix U\n");
	     return(15);
}
...
for(i=0; i<lengthFFT; i++) {
    ...
    if(V_P[i]==NULL) {
              fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate Matrix V\n");
	     return(18);
...
if(Sigma_P==NULL) {
              fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate  Matrix Sigma\n");
	     return(19);
}
...
for(i=0; i<lengthFFT; i++) {
    ...
    if(Sigma_P[i]==NULL) {
              fprintf(stderr,"cxmimo_2x2_svd_fp could not allocate vector Sigma\n");
	     return(20);
...
if(!svr_F) {
            fprintf(stderr,"cxmimo_2x2_svd_fp: Could not open  file svr.dat.\n");
                return(25);
}
for(k=0; k< lengthFFT; k++) {
    ...
    if(k==0 || (k >26 && k<38) ) continue;
     ...
     Krn_Compute2x2SVDCxMatrixFP((dsp_cxMatrix_t *)H_P, (doubleVector_t *)Sigma_P[k],(dsp_cxMatrix_t *)U_P[k],(dsp_cxMatrix_t *)V_P[k]);
...
printf("Singular values for k=%d\n",k);
     Krn_PrintVector((doubleVector_t *)Sigma_P[k],"f", stdout, 0) ;   
printf("-------------------------------------------------\n");
...
case MAIN_CODE: 
   ...
   if(feedbackState==0) {
        ...
        for(i=0; i<lengthFFT; i++) {
                   if(IT_OUT(0)) {
                         KrnOverflow("cxmimo_2x2_svd_fp",0);
                                return(99);
                    }
   ...
   for(samples = MIN_AVAIL(); samples >0; --samples) {
          ...
          if(!(k==0 || (k >26 && k<38) )) {
               ...
               for(iTx=0; iTx<numberStreams; iTx++) {
                    if(IT_OUT(iTx+1)) {
                         KrnOverflow("cxmimo_2x2_svd_fp",iTx+1);
                                return(99);
                    }
